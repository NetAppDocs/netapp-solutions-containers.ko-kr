...
이 솔루션은 VMware vSphere에 배포된 가상 어플라이언스를 사용합니다. F5 Big-IP 가상 어플라이언스의 네트워킹은 네트워크 환경에 따라 2-암 또는 3-암 구성으로 설정할 수 있습니다. 이 문서의 배포는 2-암 구성을 기반으로 합니다. Anthos와 함께 사용할 가상 어플라이언스 구성에 대한 자세한 내용은 link:https://cloud.google.com/solutions/partners/installing-f5-big-ip-adc-for-gke-on-prem["GKE 온프레미스용 F5 BIG-IP ADC 설치 가이드"^]에서 확인할 수 있습니다.
...
. Open Virtual Appliance(OVA) 파일을 link:https://downloads.f5.com/esd/serveDownload.jsp?path=/big-ip/big-ip_v15.x/15.0.1/english/virtual-edition/&sw=BIG-IP&pro=big-ip_v15.x&ver=15.0.1&container=Virtual-Edition&file=BIGIP-15.0.1-0.0.11.ALL-vmware.ova["F5 다운로드 포털"^]에서 다운로드합니다.
+
