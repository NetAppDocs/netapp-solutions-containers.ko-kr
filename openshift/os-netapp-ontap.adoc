...
NetApp ONTAP 스토리지 시스템에 대한 자세한 내용은 link:https://www.netapp.com/data-management/ontap-data-management-software/["NetApp ONTAP 웹사이트"^]을 방문하십시오.
...
ONTAP에 대한 자세한 내용은 link:https://docs.netapp.com/ontap-9/index.jsp["ONTAP 9 문서 센터"^]을 참조하십시오.
...
Amazon FSx ONTAP는 ONTAP의 널리 사용되는 데이터 액세스 및 관리 기능을 갖춘 AWS 클라우드의 완전 관리형 공유 스토리지를 제공합니다. Amazon FSx for NetApp ONTAP에 대한 자세한 내용은 link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/what-is-fsx-ontap.html["Amazon FSx for NetApp ONTAP 설명서"^]을 참조하십시오.
...
Azure NetApp Files는 Azure 네이티브 퍼스트 파티 엔터프라이즈급 고성능 파일 스토리지 서비스입니다. NetApp 계정, 용량 풀, 볼륨을 생성할 수 있는 볼륨 서비스를 제공합니다. 서비스 및 성능 수준을 선택하고 데이터 보호를 관리할 수도 있습니다. 온프레미스 환경에서 익숙하게 사용하고 있는 동일한 프로토콜과 툴을 사용하여 고성능, 고가용성, 확장 가능한 파일 공유를 생성하고 관리할 수 있습니다. Azure NetApp Files에 대한 자세한 내용은 link:https://learn.microsoft.com/en-us/azure/azure-netapp-files/["Azure NetApp Files 설명서"^]을 참조하십시오.
...
Google Cloud NetApp Volumes는 고급 데이터 관리 기능과 뛰어난 확장성을 제공하는 완전 관리형 클라우드 기반 데이터 스토리지 서비스입니다. 이를 통해 파일 기반 애플리케이션을 Google Cloud로 이전할 수 있습니다. 네트워크 파일 시스템(NFSv3 및 NFSv4.1)과 서버 메시지 블록(SMB) 프로토콜을 기본적으로 지원하므로 애플리케이션 아키텍처를 재구성할 필요 없이 애플리케이션에 영구 스토리지를 계속 사용할 수 있습니다. Google Cloud NetApp Volumes에 대한 자세한 내용은 link:https://cloud.google.com/netapp/volumes/docs/discover/overview["Google Cloud NetApp Volumes 설명서"^]을 참조하십시오.
