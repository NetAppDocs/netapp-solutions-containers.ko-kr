...
* link:https://portal.aws.amazon.com/billing/signup["AWS 계정"^]
* link:https://console.redhat.com/["Red Hat 계정"^]
* IAM 사용자 link:https://www.rosaworkshop.io/rosa/1-account_setup/["적절한 권한이 있는 경우"^] ROSA 클러스터 생성 및 액세스
...
* Helm 3 link:https://docs.aws.amazon.com/eks/latest/userguide/helm.html["선적 서류 비치"^]
...
`node-prep` 플래그를 사용하여 표시된 대로 Trident를 설치하십시오. 설치 명령을 실행하기 전에 설치 패키지를 다운로드해야 합니다. link:https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-tridentctl.html#step-1-download-the-trident-installer-package["Trident 설치 패키지를 다운로드합니다."^]을 참조하십시오.
...
Trident 백엔드 구성은 Trident가 스토리지 시스템(이 경우 FSx for ONTAP)과 통신하는 방법을 알려줍니다. 백엔드를 생성하기 위해 연결할 스토리지 가상 머신의 자격 증명과 클러스터 관리 및 NFS 데이터 인터페이스를 제공합니다. link:https://docs.netapp.com/us-en/trident/trident-use/ontap-nas.html["ontap-nas 드라이버"^]을 사용하여 FSx 파일 시스템에 스토리지 볼륨을 프로비저닝합니다.
...
----
다음 명령어를 실행하여 PVC를 생성하고 생성 여부를 확인할 수 있습니다.
